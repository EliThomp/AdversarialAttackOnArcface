...
def prepare_templates_and_genuine_probes(model, device, identity_to_paths, enroll_count):
    """
    Build:
      templates[identity] = template
      genuine_probe_paths[identity] = list of same-person probe paths
    """
    eligible = {
        identity: paths
    ...
    if len(eligible) < 1:
        raise ValueError("Need at least 1 identity with enough images.")
    ...
    return templates, genuine_probe_paths


def pgd_false_reject(
    model,
    ...
    """
    Untargeted rejection-style attack:
    minimize cosine similarity between probe embedding and correct template.

    x_clean: [1, 3, 112, 112] on device
    template: [512] on device
    """
    x_orig = x_clean.detach().clone()
    x_adv = x_clean.detach().clone()

    # x values are in [-1, 1] because of preprocess
    # if eps is defined in image-space [0,1], convert by x2
    eps_scaled = eps * 2.0
    ...
    for _ in range(steps):
        ...
        loss = torch.sum(emb * template)              # minimize similarity
        ...
        with torch.no_grad():
            ...
            # clamp to valid normalized image range [-1, 1]
            x_adv = torch.clamp(x_adv, -1.0, 1.0)

    return x_adv.detach()


def evaluate_false_reject_attack(
    model,
    device,
    templates,
    genuine_probe_paths,
    ...
    """
    Evaluate PGD false reject on genuine probes.
    ...
    for identity, probe_paths in genuine_probe_paths.items():
        template = templates[identity].to(device)
        ...
        for probe_path in probe_paths:
            ...
            # clean sample must be accepted first
            if clean_score < threshold:
                already_rejected += 1
                continue

            x_adv = pgd_false_reject(
                model=model,
                x_clean=x_clean,
                template=template,
                eps=eps,
            ...
            if max_probes is not None and total_considered >= max_probes:
                break
    ...
    if total_considered == 0:
        raise ValueError("No clean accepted genuine probes were available for attack.")

    clean_scores = np.array(clean_scores)
    adv_scores = np.array(adv_scores)

    return {
        "num_attacked_probes": total_considered,
        "num_already_rejected_clean": already_rejected,
        ...
        "avg_score_drop": float((clean_scores - adv_scores).mean()),
        "per_probe_rows": per_probe_rows,
    }

 # FOR MANUAL FALSE REJECT 
def main_manual_false_reject(
    ckpt: str,
    threshold: float,
    eps: float,
    alpha: float,
    ...
    print("\n===== MANUAL PGD FALSE REJECT =====")
    print(f"Probe image                 = {probe_img}")
    ...
    print(f"Score drop                  = {clean_score - adv_score:.4f}")
...
def main(
    ...
    templates, genuine_probe_paths = prepare_templates_and_genuine_probes(
        model=model,
        device=device,
    ...
    result = evaluate_false_reject_attack(
        model=model,
        device=device,
        templates=templates,
        genuine_probe_paths=genuine_probe_paths,
        threshold=threshold,
        eps=eps,
        alpha=alpha,
        steps=steps,
    ...
    print("\n===== PGD FALSE REJECT RESULTS =====")
    print(f"Enrollment count           = {enroll_count}")
    print(f"Threshold                  = {threshold:.4f}")
    print(f"Epsilon                    = {eps:.6f}")
    print(f"Alpha                      = {alpha:.6f}")
    print(f"Steps                      = {steps}")
    print(f"Attacked genuine probes    = {result['num_attacked_probes']}")
    print(f"Already rejected clean     = {result['num_already_rejected_clean']}")
    ...
    print(f"Average score drop         = {result['avg_score_drop']:.4f}")


if __name__ == "__main__":
    """
    Dataset mode:
    python pgd_false_reject.py <dataset_root> <ckpt> <enroll_count> <threshold> [eps] [alpha] [steps] [max_probes]

    Manual mode:
    python pgd_false_reject.py --manual <ckpt> <threshold> <eps> <alpha> <steps> <probe_img> <enroll1> [<enroll2> ...]
    """
    if len(sys.argv) >= 2 and sys.argv[1] == "--manual":
        if len(sys.argv) < 9:
            print("Usage: python pgd_false_reject.py --manual <ckpt> <threshold> <eps> <alpha> <steps> <probe_img> <enroll1> [<enroll2> ...]")
            sys.exit(1)
        ...
        probe_img = sys.argv[7]
        enrollment_imgs = sys.argv[8:]

        main_manual_false_reject(
            ckpt=ckpt,
            threshold=threshold,
            eps=eps,
            alpha=alpha,
            steps=steps,
            probe_img=probe_img,
            enrollment_imgs=enrollment_imgs,
        )
    else:
        if len(sys.argv) < 5:
            print("Usage: python pgd_false_reject.py <dataset_root> <ckpt> <enroll_count> <threshold> [eps] [alpha] [steps] [max_probes]")
            sys.exit(1)
        ...
        max_probes = int(sys.argv[8]) if len(sys.argv) > 8 else 200

        main(dataset_root, ckpt, enroll_count, threshold, eps, alpha, steps, max_probes)
